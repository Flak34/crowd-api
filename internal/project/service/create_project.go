...
func (s *Service) CreateProject(ctx context.Context, project model.Project) (int, error) {
	...
	dto := mapProjectModelToDTO(project)
	id, err := s.projectRepo.CreateProject(ctx, db, dto)
	if err != nil {
